...
% Options control the number scale, number type, number precision,
% the string's character case, commas, hyphens, trailing zeros, and more.
%
...
% -------|------------|----------------------------------------------------
%
% Note 1: For types 'cheque' and 'money' the default order is that of the
%  smallest subunit (i.e. -2). For all other types the default order is zero.
...
% Note 4: The <scale> 'indian' uses the short scale for values >= 10^21.
%
%% Money %%
%
...
switch nargin
	...
	case 2 % options in a struct
		assert(isscalar(opts)&&isstruct(opts),'Second input <opts> can be a scalar structure.')
...
if sgf<1 % round one digit to a particular order
	...
	elseif strcmp(raw,'NaN')
		...
		pwr = odr;
		[str,frc] = n2wParse(dfar,pwr,sgf,mag,raw(2:end)-48);
	end
	%
elseif isfloat(num)
	%
	...
	[str,frc] = n2wParse(dfar,pwr,sgf,mag,raw([2,4:ide-1])-48);
	%
else % integer
	%
	cls = class(num);
	...
	[str,frc] = n2wParse(dfar,pwr,sgf,mag,raw(1+isn:min(sgf+isn,end))-48);
	%
end
%
%% Money or Ordinal %%
...
if nnz(idx)>1
	error('Repeated field names:%s\b.',sprintf(' <%s>,',cFld{idx})); %#ok<SPERR>
end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%n2wCmpi
...
idx = n2wCmpi(cFld,sFld);
if any(idx)
	assert(isempty(dfar.iso),'Only one of <order> xor <sigfig> may be specified.')
	dfar.iso = strcmp(sFld,'order');
	tmp = opts.(cFld{idx});
	assert(isnumeric(tmp)&&isscalar(tmp),'The <%s> value must be a scalar numeric.',sFld)
	assert(isreal(tmp),'The <%s> value must not be complex: %g%+gi',sFld,tmp,imag(tmp))
...
if any(idx) && dfar.mny
	tmp = opts.(cFld{idx});
	assert(ischar(tmp)&&isrow(tmp),'The <%s> value must be a string (currency name).',sFld)
	spl = regexp(tmp,'\|','split');
	assert(~isempty(spl{1}),'The <%s> value string does not define a currency name.',sFld)
	assert(numel(spl)<3,'The <%s> value string may contain up to one "|" character.',sFld)
...
%
% Reshape into 3*M matrix [Hundreds;Tens;Ones]:
row = 3;
...
%
% Reshape into 2*M matrix [Tens;Ones]:
row = 2;
[mat,grp] = n2wShape(pwr,vec,row);
% Split fractional part:
[idy,frc] = n2wSplit(dfar,mat,grp,row);
...
%
% Reshape into 9*N matrix [NaN;TenLakhs;Lakhs;NaN;TenThousands;Thousands;Hundreds;Tens;Ones]:
row = 7;
