function str = num2words_rat(num,opts,varargin) %#ok<*SPERR>
...
%
% Output:
%  str = CharVector, a string with the value of <num> in words of the chosen number
...
switch nargin
	case 1
		...
		opts = struct;
	case 2
		assert(isstruct(opts)&&isscalar(opts),'Second input <opts> can be a scalar structure.')
		fnm = fieldnames(opts);
	otherwise
		fnm = [{opts},varargin(2:2:end)];
...
switch nnz(idx)
	...
	otherwise
		error('Repeated field names:%s\b',sprintf(' <%s>,',fnm{idx}));
end
%
opts.type = 'decimal';
...
if ~any(idx)
	str = sprintf('%s %s%s',str,lower(tmp),115*ones(1,ino));
else
	switch lower(opts.(fnm{idx}));
		case 'upper'
			str = sprintf('%s %s%s',str,upper(tmp),83*ones(1,ino));
		case {'lower','sentence'}
			str = sprintf('%s %s%s',str,lower(tmp),115*ones(1,ino));
		case 'title'
			str = sprintf('%s %s%s',str,tmp,115*ones(1,ino));
		otherwise
			error('Well this is rather unexpected... I don''t recognize this case.')
	end
end
%
end
