...
% |NUM2WORDS|> converts a numeric scalar into a string with the number
% value given in English words. Optional arguments control both the
% handling of the numeric input, as well as many string formatting and
% dialect options. The options are explained here together with examples.
%
...
% "order" or "sigfig" options: "sigfig" rounds the input value to the
% requested number of significant figures, while "order" rounds to the
% specified order. The default "order" is zero, which means that all
% fractional digits are rounded to give an integer. Decimal digits can
% therefore be returned by selecting an appropriate value for either the
% "order" or "sigfig" options:
...
% returned in the string. Setting the "trz" option to |true| will keep the
% trailing zeros up to the requested "order" or "sigfig":
...
num2words(11, 'type','ordinal')
num2words(111, 'type','ordinal')
%% Type: Highest
% This uses the highest multiplier/scale (i.e. only one multiplier word)
% together with a significand that includes decimal digits when required.
% In many cases it can be useful to also adjust the number of significant figures:
...
%
% * <https://en.wikipedia.org/wiki/Short_and_long_scale "short" and "long"
% scales> are explained on many websites. Most contemporary English
